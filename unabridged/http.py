...
def register_event_type_model(name: str, Model, InModel, DBModel):
    ...
    @app.get(
        f"/{model_name}/{{id}}", response_model=Model, responses={404: {"model": HTTPNotFoundError}}
    )
...
for model_name, Model, InModel, DBModel in (
    ...
    register_event_type_model(model_name, Model, InModel, DBModel)
